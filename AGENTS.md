...
- **Terraform** (`terraform.yml`) per-environment plan/apply via GitHub OIDC; staging applies, then `verify-staging` checks `GET /public/status` (a database-backed read, so it catches an apply that left the Lambda unable to reach RDS), and `apply-prod` needs both. The blocking lives in `apply-prod`'s `if:`, not its `needs:` — `!cancelled()` deliberately keeps the job alive for skipped upstreams — so `scripts/check-promotion-gate.mjs` (CI's `promotion-gate` job) evaluates that expression over a table of staging outcomes and pins `verify-staging`'s shape (`needs: apply-staging`, no `if:` — the default `success()` is what skips it after a failed apply). Edit the gate and expect that check to have an opinion. Drift detection runs daily and opens an issue on drift. It also checks the `ci-terraform` role's **live** inline policy against the AWS calls the workflows make: that role is bootstrap-only, so no pipeline reconciles it and a missing permission shows up as an AccessDenied mid-deploy. `scripts/check-ci-role-permissions.mjs` runs credential-free in CI (every `aws` call must map to an IAM action *and* to the role that workflow assumes; ci-terraform actions must be declared in bootstrap's `terraform-resources`) and with `--policy` in the daily job. Adding an `aws` call to any workflow requires adding it to that script's manifest. It carries its own self-tests for the policy evaluator and the workflow scanner — extend those rather than trusting a hand-run.
