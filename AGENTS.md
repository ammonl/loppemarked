...
- **Terraform** (`terraform.yml`) per-environment plan/apply via GitHub OIDC; staging applies, then `verify-staging` checks `GET /public/status` (a database-backed read, so it catches an apply that left the Lambda unable to reach RDS), and `apply-prod` needs both. The blocking lives in `apply-prod`'s `if:`, not its `needs:` — `!cancelled()` deliberately keeps the job alive for skipped upstreams — so `scripts/check-promotion-gate.mjs` (CI's `promotion-gate` job) evaluates that expression over every staging outcome, and `promotion-gate-rehearsal.yml` replays the same graph on dispatch with stubs. Edit the gate in either workflow and you must edit both; the check enforces it. Drift detection runs daily and opens an issue on drift.
