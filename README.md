...
- **Deploy Web (`deploy-web.yml`)** - Runs when `apps/web/**` or `packages/shared/**` change on main. Triggers an Amplify production release job and waits for the build to complete. Unlike the API and Terraform paths, this is one job with no `needs:` and no staging counterpart, so a matching push ships straight to production with nothing verified first (#314).
- **Drift Detection (`drift-detection.yml`)** - Runs daily on a cron schedule. Runs `terraform plan` for each environment and creates a GitHub issue if drift is detected. A second job covers the one thing that plan cannot: the `ci-terraform` role is declared in `infra/terraform/bootstrap`, which CI never applies and has no permission to plan (#289), so nothing reconciles it. That job reads the role's own live policy and checks it still grants the AWS calls the workflows make, opening an issue if it has fallen behind.
...
When staging has no changes of its own the apply is skipped and there is nothing to verify, so prod still proceeds — that path keys off `detect-staging`'s `has_changes` output rather than the skipped result, and requires it to be an explicit `'false'`, so a detect job that succeeds without publishing the output holds the promotion instead of opening it.

The blocking is done by `apply-prod`'s `if:` expression, not by the `needs:` list: `!cancelled()` is there so the job still runs when an upstream is skipped, which means a gate that lists `verify-staging` without testing its result would promote a broken staging and look no different in a diff. CI's `promotion-gate` job (`npm run check:promotion-gate`) reads that expression out of `terraform.yml` and evaluates it over a table of staging outcomes — verification failed, verification skipped, apply failed or skipped, no changes, run cancelled, either detect job failed before or after publishing its output — so a change to the gate that would let a failed verification through fails the pull request. It also pins the shape the table depends on: `verify-staging` must keep `needs: apply-staging` and carry no `if:`, because its default `success()` is what makes a failed apply *skip* the verification rather than run it, and the gate reads a skipped and a failed verification differently.
...
The same workflow separately checks the **CI role's live permissions**. Every other role is applied by a pipeline and so self-heals, but `ci-terraform` is bootstrap-only: the live policy is whatever an operator last applied by hand, and a permission that never made it out of the config surfaces as an AccessDenied mid-deploy — for `verify-staging` that blocks every production apply. `npm run check:ci-permissions` runs the credential-free half locally: every `aws` call in a workflow must be mapped to an IAM action and to the role that workflow assumes for it, so a new call cannot be added without deciding which permission it needs. Actions used with the `ci-terraform` role must additionally be declared in bootstrap's `terraform-resources` document — the one the daily job actually fetches. `ci-deploy` actions are recorded but not checked against a policy, since the environment applies reconcile that role. The daily job adds the live comparison, and it models only Allow/Deny over `Action` and `Resource`: a statement using `NotAction`, `NotResource`, or a `Condition` is reported as unevaluable rather than assumed to pass. Fixing a reported gap means applying `infra/terraform/bootstrap` with operator credentials — CI cannot do it.
